<?php
    ...
    Plugin URI: https://www.powr.io/plugins/facebook-feed
    Description: Get more Likes on Facebook with an automatic FB feed  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Facebook_Feed extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Facebook Feed.
        </p>
        <p>
    ...
    function powr_facebook_feed_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-facebook-feed' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-facebook-feed' label='$label'></div>";
      }else{
      	"<div class='powr-facebook-feed'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_facebook_feed_menu' );
    function powr_facebook_feed_menu() {
      add_menu_page( 'POWr Facebook Feed', 'POWr Facebook Feed', 'manage_options', 'powr-facebook-feed-settings', 'powr_facebook_feed_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_facebook_feed_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/facebook-feed';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/facebook-feed';
      }
    ...
    register_activation_hook(__FILE__, 'powr_facebook_feed_plugin_activate');
      add_action('admin_init', 'powr_facebook_feed_plugin_redirect');

      function powr_facebook_feed_plugin_activate() {
      add_option('powr_facebook_feed_plugin_do_activation_redirect', true);
      }
      ...
      function powr_facebook_feed_plugin_redirect() {
      if (get_option('powr_facebook_feed_plugin_do_activation_redirect', false)) {
          delete_option('powr_facebook_feed_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-facebook-feed-settings&' );
          }
       }
      }
